...
| 15a | The PR branch is checked out in the **main working tree**, no linked worktree exists | **Case B.** `$WT_PATH` equals `$MAIN_WT`, so there is no worktree to remove. | `git worktree list` lists the main working tree first, with its own `branch` line. Detecting on that match alone sends every non-worktree PR into Case A, where `worktree remove` exits 128 (`is a main working tree`) and teardown stops: branch, remote branch and `pull` all skipped, after a merge that succeeded. |
| 15b | The PR branch is in a linked worktree, and `<workspace>` is the main working tree | **Case A**, `$WT_PATH` = the linked path. | The workspace need not be the checkout that holds the branch; the comparison, not the argument, decides. |
| 15c | The PR branch is in a linked worktree, and `<workspace>` is that worktree | **Case A**, identically. | `MAIN_WT` is the first entry of the list whichever member of the repo you ask, so both spellings agree. |
| 15d | No checkout in this repository holds the branch | **`NEITHER`** — skip teardown, report, go to Step 5. | Same rule as case 12, now an explicit third outcome rather than the `else` of a two-way test. |
